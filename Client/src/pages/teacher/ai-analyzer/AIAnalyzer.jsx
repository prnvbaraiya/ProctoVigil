...
function AIAnalyzer() {
    ...
    useEffect(() => {
        ...
        const getData = async () => {
            ...
            const filteredTests = [];
            quiz.data.map((test) => {
                if (test.author._id == userData.data[0]._id) filteredTests.push(test._id);
            });
    ...
    const getMouthData = async () => {
        return await axios.get('http://127.0.0.1:5001/flask/' + d.split('/').slice(-1))
    }
    const getEyeData = async () => {
        return await axios.get('http://127.0.0.1:5002/flask/' + d.split('/').slice(-1))
    }
    const getPhoneData = async () => {
        return await axios.get('http://127.0.0.1:5003/flask/' + d.split('/').slice(-1))
    }

    const [videoAnalyzationDataMouth, setVideoAnalyzationDataMouth] = useState(null)
    const [videoAnalyzationDataEye, setVideoAnalyzationDataEye] = useState(null)
    const [videoAnalyzationDataPhone, setVideoAnalyzationDataPhone] = useState(null)

    const getVideoAnalyzed = async () => {
        setLoading(true)
        console.log(d.split('/').slice(-1))

        const response = await Promise.all([getMouthData(), getEyeData(), getPhoneData()])

        console.log(
            response[0].data, response[1].data, response[2].data,
            typeof (response[0].data)
        )
        if (Object.keys(response[0].data).includes('error') || Object.keys(response[1].data).includes('error') || Object.keys(response[2].data).includes('error')) {
            setLoading(false)
    ...
    const getListItem = (arr) => {
        ...
        if (arr.length != 0) {
            ...
            listItem = arr.map((occr) =>
                <div className="timestamp" >at {occr} second</div>
            )
        }
        else {
            listItem = "None"
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td className="timestamp-container">
                                            ...
                                            {videoAnalyzationDataEye['Total'] ? videoAnalyzationDataEye['Total'] : ''}
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                            <table className="category">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        {console.log(videoAnalyzationDataPhone['PhoneDetected'], '>>>>>>>>>>>....')}
                                        <td className="category-title" >
                            ...
                            <table className="category">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td className="timestamp-container">
                                            {
                                                getListItem(videoAnalyzationDataPhone['MoreThanOnePersonDetected'])
                                            }
                                        </td>
                                    </tr>
                                </tbody>
                            ...
                            <table className="category">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td className="timestamp-container">
                                            {
                                                getListItem(videoAnalyzationDataPhone['NoPersonDetected'])
                                            }
